...
static GLfloat v_Cube[8][3] = // Tọa độ các điểm trên hình hộp
{
    {0.0, 0.0, 0.0}, //A
    {2.0, 0.0, 0.0}, //B    
    {0.0, 2.0, 0.0}, //D
...
static GLubyte quadIndices[6][4] = // thứ tự các mặt trong hình hộp
{
    {1, 4, 7, 6}, // mặt BCGF
    {7, 4, 2, 5}, // GCDH
    {3, 5, 2, 0}, // EHDA
...
// Tọa độ pháp tuyến của 3 điểm trong không gian
static void getNormal3p(GLfloat x1, GLfloat y1, GLfloat z1, GLfloat x2, GLfloat y2, GLfloat z2, GLfloat x3, GLfloat y3, GLfloat z3);
void cube(GLfloat colr1, GLfloat colr2, GLfloat colr3); // vẽ khối hộp
void table();// vẽ bàn
void chair();// vẽ ghế
void full_set(); // 1 bộ bàn ghế
void nine_full_set_chair_table(); // 9 bộ bàn ghế trong lớp học
void floor();// vẽ sàn
void wall(GLfloat col1, GLfloat col2, GLfloat col3); // vẽ tường
void closet(); // vẽ tủ 
void window(); // cửa sổ
void door(); // cửa ra vào
void room(); // vẽ căn phòng 
void fan_leg(); // cánh quạt
void fan_face(); // vẽ khớp nối của quạt
void fan_stand(); // trục của quạt
void fan_set(); // 1 bộ quạt 
void fan(); // sự kiện quạt
void clock(); // đồng hồ
void classRoom(); // Tổng hợp tất cả các nội thất của lớp học 
void light(); // sự kiện đèn
static void resize(int width, int height); // hàm điều chỉnh lại khung hình khi cửa sổ bị thay đổi kích thước
void display(void); // display func
void myKeyboardFunc(unsigned char key, int x, int y); // sự kiện bàn phím
void animate(); //Hàm xử lý khi cửa sổ ở chế độ tĩnh
...
int main(int argc, char** argv)
{
    ...
    glShadeModel(GL_SMOOTH); // chế độ shading
    ...
    glutIdleFunc(animate); //Hàm xử lý khi cửa sổ ở chế độ tĩnh
...
(GLfloat x1, GLfloat y1, GLfloat z1, GLfloat x2, GLfloat y2, GLfloat z2, GLfloat x3, GLfloat y3, GLfloat z3)
{
    ...
    // tọa độ vecto thứ nhất
    Ux = x2 - x1;
    Uy = y2 - y1;
    Uz = z2 - z1;

    // tọa độ vecto thứ hai
    Vx = x3 - x1;
    Vy = y3 - y1;
    Vz = z3 - z1;

    // tọa độ vecto pháp tuyến = tọa độ của tích có hướng của hai vecto trên
    Nx = Uy * Vz - Uz * Vy;
    Ny = Uz * Vx - Ux * Vz;
    Nz = Ux * Vy - Uy * Vx;
...
void cube(GLfloat colr1, GLfloat colr2, GLfloat colr3) // vẽ khối hộp
{
    ...
    glMaterialfv(GL_FRONT, GL_AMBIENT, cube_amb); // chiếu sáng toàn phần
    glMaterialfv(GL_FRONT, GL_DIFFUSE, cube_dif); // chiếu sáng khuếch tán
    glMaterialfv(GL_FRONT, GL_SPECULAR, cube_spec); // chiếu sáng phản xạ
    glMaterialfv(GL_FRONT, GL_SHININESS, cube_sh); // cường độ điểm chiếu sáng phản xạ

    glBegin(GL_QUADS); // chế độ vẽ tứ giác
    for (GLint i = 0; i < 6; i++) // 6 mặt
    {
...
void table()
{
    ...
    glPushMatrix();
    glScalef(table_width, table_length, table_height); //mặt bàn
    glTranslatef(-1, -1, -1);
    cube(0.53, 0.39, 0.28);
    glPopMatrix();
...
void chair()
{
    ...
    glPushMatrix();
    glScalef(chair_width, chair_length, chair_height); // mặt ngồi
    glTranslatef(-1, -1, -1);
...
void closet()
{
    ...
    GLfloat closet_height = 15, closet_width = 8, closet_length = 5;


    glPushMatrix();  //main_box
    glScalef(closet_width, closet_height, closet_length);
    glTranslatef(-1, -1, -1);
    cube(0.294, 0.000, 0.510);
    glPopMatrix();

    glPushMatrix(); // đường kẻ ở giữa
    glTranslatef(0.4, 0, 4.1);
    glScalef(closet_width / 45, (closet_height - 0.5), closet_length / 5.5);
    glTranslatef(-1, -1, -1);
    cube(1.000, 0.000, 1.000);
    glPopMatrix();

    glPushMatrix(); // tay cầm bên phải
    glTranslatef(1.5, 5, 4.5);
    glScalef(closet_width / 25, closet_height / 15, closet_length / 5.5);
    glTranslatef(-1, -1, -1);
    cube(0.957, 0.643, 0.376);
    glPopMatrix();

    glPushMatrix(); // tay cầm bên trái
...
void window()
{
    ...
    glPushMatrix(); // window_lock

    glTranslatef(44, 6, -13);
    glRotatef(90, 0, 1, 0);
    glTranslatef(-0.6, 5, 4.5);
    glScalef(0.40, 1.5, 0.5 / 5.5);
    glTranslatef(-1, -1, -1);
    ...
    glPushMatrix();  //out of uindow
    glTranslatef(48.5, 10, -17);
    glRotatef(90, 0, 1, 0);
    glScalef(3, 10, 0.5);
    glTranslatef(-1, -1, -1);
    cube(0.0, 0.0, 0.0);
...
void display(void)
{
    ...
    glLoadIdentity();
    glFrustum(-3, 3, -3, 3, 2.0, 100.0); // chiếu phối cảnh
